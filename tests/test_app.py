...
def test_explain_wrong_model_extension(client):
    data = {
        'form_type': 'global_form',
        'xtest': (BytesIO(b"num\n1"), 'test.csv'),
        'model': (BytesIO(b"x"), 'model.txt')
    }
...
def test_explain_local_obs_errors(client):
    ...
    response = client.post('/explain', content_type='multipart/form-data', data={
        'form_type': 'global_form',
        'xtest': (BytesIO(csv_content), 'xtest.csv'),
        'model': (BytesIO(onnx_content), 'model.onnx')
    })
